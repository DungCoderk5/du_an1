...
function nguoidung_insert($username, $email, $password, $address, $phone, $account_status = 1, $role,$avatar) {
    // Mã hóa mật khẩu
    // $hashed_password = password_hash($password, PASSWORD_DEFAULT);

    $sql = "INSERT INTO users (username, email, password, address, phone, account_status, role,avatar) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
    
    return pdo_execute($sql, $username, $email,$password, $address, $phone, $account_status, $role,$avatar);
}

function nguoidung_update($user_id, $username, $email, $password, $address, $phone, $role, $account_status,$avatar) {
    $sql = "UPDATE users SET username = ?, email = ?, password = ?, address = ?, phone = ?, role = ?, account_status = ?, avatar = ? WHERE user_id = ?";
    // $hashed_password = password_hash($password, PASSWORD_DEFAULT);
    return pdo_execute($sql, $username, $email,$password, $address, $phone, $role, $account_status,$avatar, $user_id);
}
...
function nguoidung_show($user_id) {
    $sql = "UPDATE users SET account_status = 1 WHERE user_id = ?";
    return pdo_execute($sql, $user_id);
}
